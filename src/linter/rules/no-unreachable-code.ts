...
import type { Program, Statement, Expression, KeywordStatement } from '../../parser/ast.js';
 ...
 */
function evaluateConstantExpression(expr: Expression): boolean | undefined {
...
export const noUnreachableCode: LintRule = {
  ...
  create(context: RuleContext) {
     ...
     */
    function isLabel(stmt: Statement): boolean {
     ...
     */
    function isUnconditionalTransfer(stmt: KeywordStatement): boolean {
      ...
      for (let i = 0; i < statements.length; i++) {
        ...
        if (stmt.type === 'KeywordStatement') {
          ...
          if (stmt.keyword === 'if' || stmt.keyword === 'elseif') {
            if (stmt.argument) {
              const conditionValue = evaluateConstantExpression(stmt.argument as Expression);

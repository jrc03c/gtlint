...
import type { KeywordStatement } from '../../parser/ast.js';
 ...
 */
export const noEmptyBlocks: LintRule = {
  ...
  create(context: RuleContext) {
    return {
      KeywordStatement(node: KeywordStatement) {
        if (node.body.length > 0 || node.subKeywords.length > 0) return;

...
interface DisableRegion {
  startLine: number;
  endLine: number | null; // null means until end of file
...
export function parseDirectives(source: string): DirectiveState {
  ...
  const activeDisables = new Map<string, number>();

  // Track next-line disables
  let nextLineDisable: Set<string> | 'all' | null = null;
 ...
 */
export function isExpectedVar(state: DirectiveState, varName: string): boolean {
 ...
 */
export function isReturnedVar(state: DirectiveState, varName: string): boolean {

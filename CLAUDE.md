...
- Configurable spaces inside braces (`spaceInsideBraces`), brackets (`spaceInsideBrackets`), and parentheses (`spaceInsideParens`) — numeric values (0 = no spaces, 1 = single space, etc.); empty pairs are never padded; string interpolation braces in text are unaffected
